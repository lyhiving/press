...
press.define(function (require) {
	...
	return {
		...
		__reset: function (area) {
			...
			form.find('.press-area-field-target')[0].selectedIndex = index;
		...
		__getContent: function (module) {
			...
			var option = this.__option(module);
		...
		__upload: function (file, callback) {
			...
			$.ajax({
				dataType: 'json',
				url: 'http://tms.taobao.com/photo/upload.htm?_input_charset=utf-8',
				type: 'post',
				data: formdata,
				processData: false,
				contentType: false,
		...
		__setAreas: function (module) {
			...
			$.ajax({
				type: 'post',
				url: press.api + '?method=tms.module.plugin.setArea',
				data: {
					page: press.page,
					guid: module.parents('.J_Module').attr('data-guid'),
					data: JSON.stringify({
						'option': [self.__option(module)],
						'list': list
					}),
