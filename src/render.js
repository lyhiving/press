...
press.define(function (require) {
	...
	var render = {
		...
		setModuleAdmin: function (module, width) {
			...
			var _module = $(module), _template;

			// 检查是否为预置模块类型
			if (_module.attr('data-name') === 'press-area') {
				_template = template.TEMPLATE_AREA_ADMIN;
			} else {
				_template = template.TEMPLATE_MODULE_ADMIN;
			}

			_module.append(mustache.render(_template, {
				width: width,
				valid: $.parseJSON(press.role).valid
			}));
